...
contract TokenTransfer is OwnerIsCreator {
    ...
    uint256 public s_allowedChain;

    /// @notice Constructor initializes the contract with the router address.
    /// @param _router The address of the router contract.
    /// @param _link The address of the link contract.
    constructor(address _router, address _link, uint256 _allowedChain) {
        s_router = IRouterClient(_router);
        s_linkToken = IERC20(_link);
        s_allowedChain = _allowedChain;
    }

    /// @dev Modifier that checks if the chain with the given destinationChainSelector is allowlisted.
    /// @param _destinationChainSelector The selector of the destination chain.
    modifier onlyAllowlistedChain(uint64 _destinationChainSelector) {
        if (s_allowedChain != _destinationChainSelector) {
    ...
    function transferTokensPayNative(
        ...
        address _token,
        uint256 _amount
    ) external onlyAllowlistedChain(_destinationChainSelector) 
    returns (bytes32 messageId)
    {
        ...
        // Approve the contract to spend tokens on behalf of msg.sender
